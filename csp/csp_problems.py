...
def sudokuCSP(initial_sudoku_board, model='neq'):
    ...
    for row in var_array:
        ...
        if model == 'neq':
            ...
            constraint_list.append(AllDiffConstraint(f'RowConst({r})', row))
    ...
    for colj in range(len(var_array[0])):
        ...
        if model == 'neq':
            ...
            constraint_list.append(AllDiffConstraint(f'RowConst({c})', scope))
    ...
    for i in [0, 3, 6]:
        for j in [0, 3, 6]:
            ...
            if model == 'neq':
                ...
                constraint_list.append(AllDiffConstraint(f'SubsquareConst({(i/3)+1}, {(j/3)+1})', scope))
...
# helper function for 'solve_planes(...)'
# returns:
#   constraints [] -> constraing list for CSP problem
#   var_list []
#
def solve_planes_generate_constraints(planes_problem):
    var_list = []
    constraints = []
    ...
    for plane in planes_problem.planes:
        for fly_idx in range(len(planes_problem.can_fly(plane))):
            name = f'Plane_{plane}_Flight_{fly_idx + 1}'

            if fly_idx == 0:
                domain = planes_problem.can_start(plane)
            else:
                domain = planes_problem.can_fly(plane)
            domain = domain + ['null']

            var = Variable(name, domain)
            var_list.append(var)
            var_constraints[name] = var

    # Table constraints for each plane in which a sequence exists
    for plane in planes_problem.planes:
        for fly_idx in range(len(planes_problem.can_fly(plane))):
            # idx 'can-fly' flights by '1'
            cur_fly_idx, next_fly_idx = fly_idx + 1, fly_idx + 2
            
            if f'Plane_{plane}_Flight_{next_fly_idx}' not in var_constraints.keys():
                break

            fly_var, next_fly_var = var_constraints[f'Plane_{plane}_Flight_{cur_fly_idx}'], var_constraints[f'Plane_{plane}_Flight_{next_fly_idx}']

            possible_flights = list(map(lambda x: list(x), planes_problem.can_follow))
            final_constraint = [flight_path for flight_path in possible_flights if (flight_path[0] in fly_var.domain() and flight_path[1] in next_fly_var.domain())]
            final_constraint += [[flight, 'null'] for flight in fly_var.domain()]

            # Add to constraints
            constraints.append(TableConstraint('Sequence_'+f'Plane_{plane}_Flight_{cur_fly_idx}'+f'Plane_{plane}_Flight_{next_fly_idx}', [fly_var, next_fly_var], final_constraint))

    # NValuesConstraint -> flight uniqueness constraints
    for flight in planes_problem.flights:
        # Add to constraints
        constraints.append(NValuesConstraint(f'Unique_{flight}', var_list, [flight], 1, 1))

    # Maintenance constraints
    for plane in planes_problem.planes:
        maintenance_iteration = len(planes_problem.can_fly(plane)) - planes_problem.min_maintenance_frequency + 1
        
        # sliding window: iterate through sections of flights of size min_maintenance_frequency
        for maintenance_idx in range(0, maintenance_iteration):
            maintenance_scope = []
            for freq_idx in range(0, planes_problem.min_maintenance_frequency):
                maintenance_scope.append(var_constraints[f'Plane_{plane}_Flight_{maintenance_idx + freq_idx + 1}'])

            # Add to constraints
            constraints.append(NValuesConstraint(f'Maintenance_{plane}', maintenance_scope, ['null'] + planes_problem.maintenance_flights, 1, planes_problem.min_maintenance_frequency))

    return constraints, var_list
...
def solve_planes(planes_problem, algo, allsolns,
    ...
    constraints, var_list = solve_planes_generate_constraints(planes_problem)

    csp = CSP(f'PlaneCSP', var_list, constraints) #set to to your CSP
    solutions, _ = bt_search(algo, csp, variableHeuristic, allsolns, trace)

    # Filter through solutions
    solve_planes_return = []
    for soln in solutions:
        solution = []
        solution_dict = defaultdict(list)
        for var in soln:
            if var[1] != 'null': solution_dict[var[0].name().split('_')[1]].append(var[1])
        for key, values in solution_dict.items():
            solution.append([key] + values)

        solve_planes_return.append(solution)

    return solve_planes_return