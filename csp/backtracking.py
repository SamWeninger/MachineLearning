...
def FC(unAssignedVars, csp, allSolutions, trace):
    ...
    nxtvar = unAssignedVars.extract()
    if trace: print("==>Trying {}".format(nxtvar.name()))
    for val in nxtvar.curDomain(): # look at remaining domain only
        if trace: print("==> {} = {}".format(nxtvar.name(), val))
        nxtvar.setValue(val)
        constraintsOK = True
        for cnstr in csp.constraintsOf(nxtvar):
        ...
        if constraintsOK:
            ...
            if new_solns:
                solns.extend(new_solns)
            if len(solns) > 0 and not allSolutions:
                # restore pruned variables
                for var in csp.variables():
                    var.restoreValues(nxtvar, val)
                break  #don't bother with other values of nxtvar
                       #as we found a soln.
        ...
        for var in constraint.scope():
            for val in var.curDomain():
                if not constraint.hasSupport(var, val):
                    ...
                    if var.curDomainSize() == 0:
                        while pruned:
                            pruned.pop().restoreValues(reasonVar, reasonVal)
                        return "DWO"
                    other_constraints = [other_constraint for other_constraint in csp.constraintsOf(var) if (constraint != other_constraint and other_constraint not in constraints)]
...
def GAC(unAssignedVars, csp, allSolutions, trace):
    ...
    for val in nxtvar.curDomain(): # look at remaining domain only
        ...
        if GacEnforce(csp.constraints(), csp, nxtvar, val) == "DWO":
            constraintsOK = False
            if trace: print("<==falsified constraint\n")
