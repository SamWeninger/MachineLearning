...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        # qi, qj -> class Variable
        for vali in qi.domain():
            for valj in qj.domain():
                if self.queensCheck(vali, valj):
                    constraints.append([vali, valj])

        TableConstraint.__init__(self, self._name, scope, constraints)
...
class NValuesConstraint(Constraint):
    ...
    def check(self):
        ...
        req_cnt = sum(1 for i in assignments if i in self._required)
        return self._lb <= req_cnt <= self._ub
    ...
    def hasSupport(self, var, val):
        ...
        def NvalsNotEqual(l):
            ...
            req_cnt = sum(1 for i in vals if i in self._required)

            if self.scope_len - len_vals == 0:
                return self._lb <= req_cnt <= self._ub
            if self.scope_len - len_vals + req_cnt < self._lb or req_cnt > self._ub:
                return False
            return True

        varsToAssign = self.scope()
        varsToAssign.remove(var)
        x = self.Nfindvals(varsToAssign, [(var, val)], NvalsNotEqual, NvalsNotEqual)
        return x

    def Nfindvals(self, remainingVars, assignment, finalTestfn, partialTestfn=lambda x: True):
        '''Helper function for finding an assignment to the variables of a constraint
        that together with var=val satisfy the constraint. That is, this
        function looks for a supporing tuple.

        Nfindvals uses recursion to build up a complete assignment, one value
        from every variable's current domain, along with var=val.
        ...
        #sort the variables call the internal version with the variables sorted
        remainingVars.sort(reverse=True, key=lambda v: v.curDomainSize())
        return self.Nfindvals_(remainingVars, assignment, finalTestfn, partialTestfn)

    def Nfindvals_(self, remainingVars, assignment, finalTestfn, partialTestfn):
        '''Nfindvals_ internal function with remainingVars sorted by the size of
        their current domain'''
        if len(remainingVars) == 0:
            return finalTestfn(assignment)
        var = remainingVars.pop()
        for val in var.curDomain():
            assignment.append((var, val))
            if partialTestfn(assignment):
                if self.Nfindvals_(remainingVars, assignment, finalTestfn, partialTestfn):
                    return True
            assignment.pop()   #(var,val) didn't work since we didn't do the return
        remainingVars.append(var)
        return False